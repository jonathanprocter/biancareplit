...
class AdaptiveLearningSystem:
    ...
    def generate_adaptive_content(self, user_id, topic=None):
        ...
        try:
            ...
            patterns = AdaptivePattern.query.filter_by(user_id=user_id)\
    ...
    def _analyze_study_times(self, reviews):
        ...
        for review in sorted(reviews, key=lambda x: x.created_at):
            if last_time and (review.created_at - last_time).seconds > 1800:  # 30 min break
    ...
    def _evaluate_topic_mastery(self, reviews):
            ...
            if review.content and review.content.category:
                ...
                if review.content.difficulty:
                    ...
                    if diff_level in stats['difficulty_performance']:
                        stats['difficulty_performance'][diff_level].append(review.is_correct)
        ...
        for topic, stats in topic_stats.items():
            ...
            recent_accuracy = (sum(stats['recent_performance']) / len(stats['recent_performance'])) * 100
            ...
            if overall_accuracy < 70 or recent_accuracy < 60:
                weak_areas.append({
                    'topic': topic,
                    'reason': 'Low accuracy' if overall_accuracy < 70 else 'Recent struggles',
                    'accuracy': overall_accuracy
                })
            elif overall_accuracy > 85 and recent_accuracy > 80:
                strong_areas.append({
                    'topic': topic,
                    'mastery_level': 'Advanced' if difficulty_mastery.get('advanced', 0) > 70 else 'Intermediate',
                    'accuracy': overall_accuracy
                })
    ...
    def _determine_learning_style(self, reviews):
        ...
        for style, stats in content_performance.items():
            if stats['total'] > 0:
                ...
                effectiveness = (accuracy * 0.7) + ((1000 - min(avg_time, 1000)) * 0.3 / 10)
                style_effectiveness[style] = effectiveness
                
        # Determine primary style
        primary_style = max(style_effectiveness.items(), key=lambda x: x[1])[0] if style_effectiveness else 'visual'
    ...
    def _categorize_content_type(self, content):
        ...
        interactive_keywords = {'simulation', 'practice', 'exercise', 'interactive', 'hands-on'}
