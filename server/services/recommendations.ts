...
export async function generatePersonalizedPath(userId: number): Promise<typeof learningPaths.$inferSelect> {
  ...
  const [learningPath] = await db.insert(learningPaths)
    .values({
      ...
      estimatedCompletionTime: recommendedCourses.reduce((sum, course) => 
        sum + course.estimatedHours * 60, 0
      ),
    })
    .returning();
...
async function findRecommendedCourses(factors: RecommendationFactors) {
  ...
  const scoredCourses = availableCourses.map(course => {
    ...
    const topicMatchScore = factors.preferredTopics
      .filter(topic => topics.includes(topic))
      .length;

    // Calculate time fit score
    const timeMatchScore = Math.abs(
      factors.availableTimeMinutes - course.estimatedHours * 60
    );
