...
class AutoscaleManager:
    ...
    def check_metrics(self) -> bool:
        ...
        try:
            ...
            if cpu_percent > target_cpu and self.current_instances < self.config['max_instances']:
                self._scale_up()
                return True
            elif cpu_percent < target_cpu/2 and self.current_instances > self.config['min_instances']:
                self._scale_down()
                return True
