            ...
            for middleware_name, settings in config.items():
                for field in required_fields:
                    if field not in settings:
                        logger.error(f"Missing required field '{field}' in middleware '{middleware_name}'")
                        return False
                        
                if not isinstance(settings['enabled'], bool):
                    logger.error(f"'enabled' must be boolean in middleware '{middleware_name}'")
                    return False
                    
                if not isinstance(settings['order'], int):
                    logger.error(f"'order' must be integer in middleware '{middleware_name}'")
                    return False
