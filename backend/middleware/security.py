...
class SecurityMiddleware(BaseMiddleware):
    ...
    def _configure(self, **config) -> None:
        ...
        self.request_size_limit = config.get('request_size_limit', 10 * 1024 * 1024)  # 10MB default
    ...
    def process_request(self, request: Request) -> Optional[Response]:
        ...
        if request.content_length and request.content_length > self.request_size_limit:
            logger.warning(f"Request size {request.content_length} exceeds limit {self.request_size_limit}")
