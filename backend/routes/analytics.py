...
def get_performance_metrics():
    ...
    try:
        user_id = request.args.get('userId', 'default_user') # Provide default user for development
        if not user_id:
        ...
        try:
            start_date = datetime.fromisoformat(start_date) if start_date else datetime.now() - timedelta(days=30)
...
def generate_analytics_report():
    ...
    try:
        ...
        with db.session.begin():
            ...
            study_stats = db.session.execute(text("""
                SELECT 
            ...
            question_stats = db.session.execute(text("""
                SELECT 
            ...
            improvement = db.session.execute(text("""
    ...
    try:
        ...
        try:
            ...
            with db.session.begin():
                ...
                daily_progress = db.session.execute(
                    ...
                    LIMIT 7
                    """),
                    {"user_id": user_id, "start_date": datetime.now() - timedelta(days=7)}
                ).fetchall()
        ...
        dashboard_data = {
            'summary': {
                ...
                'mastery': round(average_score / 20, 2)  # Simplified mastery calculation
            },
            'progress': {
                'daily': [{
    ...
    try:
        ...
        if performance_trend:
            weak_categories = sorted(
                performance_trend,
                key=lambda x: x[1]  # Sort by avg_score
            )[:2]
        ...
        if PROMETHEUS_AVAILABLE:
            REQUEST_COUNT.labels(
                method='GET',
                endpoint='/analytics/daily-summary',
                status=200
            ).inc()
...
def metrics():
    ...
    if not PROMETHEUS_AVAILABLE:
        ...
        return jsonify({
            'status': 'error',
            'message': 'Metrics collection is not available'
        }), 503
