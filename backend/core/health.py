...
class HealthCheck:
    ...
    def get_system_health(self):
        ...
        if (
            self._last_check
            and (now - self._last_check).total_seconds() < self._cache_duration
        ):
    ...
    def _check_middleware():
        ...
        try:
            ...
            for name, middleware in middleware_registry._middleware.items():
                try:
                    ...
                    if hasattr(middleware, "get_health"):
                        ...
                        # Basic status check
                        is_initialized = bool(
                            getattr(middleware, "_initialized", False)
                        )
            ...
            return {
                ...
                "message": (
                    "All middleware healthy"
                    if all_healthy
                    else "Some middleware components degraded"
                ),
    ...
    def _update_metrics(health_status):
        ...
        try:
            ...
            metrics_manager.system_health.set(
...
def health():
    ...
    try:
        ...
        status_code = (
            200 if is_healthy else 503 if health_status["status"] == "degraded" else 500
        )
