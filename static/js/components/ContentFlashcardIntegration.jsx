...
class ErrorBoundary extends React.Component {
  ...
  componentDidCatch(error, errorInfo) {
    ...
    window.dispatchEvent(new CustomEvent('flashcardSystemError', {
      detail: {
        error: error.message,
        stack: error.stack,
...
const ContentFlashcardIntegration = () => {
  ...
  const updateProgress = useCallback((completedCards, accuracy = 0, categories = {}, slots = []) => {
    try {
      ...
      const validCategories = typeof categories === 'object' && categories !== null 
      ...
      const completionProgress = validCompletedCards > 0
      ...
      const slotProgress = validSlots.length > 0 
      ...
      const fallbackProgress = Math.min(100, Math.max(0, Math.round((validCompletedCards / BASE_TARGET) * 100)));
      setProgress(fallbackProgress);
      return { fallbackProgress };
    }
  }, []);

  const initializeSystem = useCallback(async (retryCount = 0) => {
    const MAX_RETRIES = 3;
  ...
  const updateAnalytics = useCallback(async () => {
    ...
    try {
      ...
      if (inactiveTime > 0) {
        ...
        window.dispatchEvent(new CustomEvent('flashcardSystemInactivity', {
          detail: { 
      ...
      const stdDev = Math.sqrt(
        sessionDurations.reduce((sum, duration) => 
          sum + Math.pow(duration - averageSessionDuration, 2), 0
      ...
      const analyticsPayload = {
        ...
        categoryProgress: Object.entries(analytics.categoryProgress || {})
      ...
      setAnalytics(prev => {
        ...
        return {
          ...prev,
          totalStudyTime: Math.max(prev.totalStudyTime, Number(updatedAnalytics.totalStudyTime) || 0),
          completedCards: Math.max(prev.completedCards, Number(updatedAnalytics.completedCards) || 0),
      ...
      window.dispatchEvent(new CustomEvent('flashcardSystemAnalyticsSuccess', {
        detail: {
          timestamp: now,
          slotId: currentSlot.id,
      ...
      window.dispatchEvent(new CustomEvent('flashcardSystemAnalyticsError', {
        detail: { 
          error: err.message,
          timestamp: Date.now(),
          componentState: {
