          ...
          >
            ...
            {showTooltip && (
              <div className="absolute bg-white p-2 rounded-md shadow-lg border text-xs max-w-xs">
                {flashcard.metadata?.difficulty === 'BEGINNER' &&
                  'Foundational concepts and basic knowledge questions'}
                {flashcard.metadata?.difficulty === 'INTERMEDIATE' &&
                  'Application of knowledge and moderate complexity scenarios'}
                {flashcard.metadata?.difficulty === 'ADVANCED' &&
                  'Complex scenarios requiring deep understanding and critical thinking'}
              </div>
            )}
          </div>
        </div>
  ...
  return (
    <div className="p-4">
      ...
      {currentCards.length > 0 ? (
        <FlashcardReview flashcard={currentCards[0]} onReview={handleReview} />
      ) : (
