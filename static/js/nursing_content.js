...
class NursingContentHandler {
  ...
  async submitAnswer() {
    try {
      ...
      const timing = this.timer
        ? this.timer.getTiming()
        : {
            time_taken: 60,
            study_duration: 60,
          };
      ...
      if (!result.correct && window.flashcardSystem) {
        try {
          const flashcardData = {
            ...
            back: `Correct Answer: ${
              this.currentQuestion.options[result.correct_answer]
                      ...
                      result.correct ? 'feedback-success' : 'feedback-error'
                        ...
                        <div class="feedback-content">
                            <p><strong>Your Answer:</strong> Option ${String.fromCharCode(
                              65 + parseInt(requestData.selected_option),
                            )}</p>
  ...
  showNotification(message, type = 'error') {
    try {
      ...
      const icons = {
        ...
        warning:
          '<svg class="w-6 h-6 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>',
      };
