class EnhancedFlashcardSystem {
    ...
    async createCustomFlashcard(front, back, difficulty = 'medium', category = 'nursing', nclexCategory = null, keywords = []) {
        try {
            if (!this.initialized) {
            ...
            const isDuplicate = this.flashcards.some(card => 
                card.front === cleanFront || card.back === cleanBack
            );
    ...
    render() {
        ...
        if (this.flashcards.length === 0) {
            this.displayContainer.innerHTML = '<p>No flashcards available yet. Complete some practice questions to generate flashcards!</p>';
            return;
        }
            ...
            <div class="flashcard">
                                ...
                                ${currentCard.clinicalNotes ? `
                                    <div class="clinical-notes">
                                        ...
                                        <div class="clinical-content">
                                            ${currentCard.clinicalNotes.split('\n').map(note => 
                                                `<p>${note}</p>`).join('')}
                                        </div>
                                    </div>
                                ` : ''}
                ...
                <div class="flashcard-info">
                    ...
                    ${currentCard.lastReviewed ? 
                        `<p>Last Reviewed: ${new Date(currentCard.lastReviewed).toLocaleDateString()}</p>` : 
