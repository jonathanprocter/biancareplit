...
class PerformanceMiddleware extends BaseMiddleware {
    ...
    updateAggregateMetrics(metrics) {
        ...
        const oldTotal = this.aggregateMetrics.averageDuration * (this.aggregateMetrics.totalRequests - 1);
        this.aggregateMetrics.averageDuration = (oldTotal + metrics.duration) / this.aggregateMetrics.totalRequests;
    ...
    async emitPerformanceWarning(metrics) {
        try {
            ...
            if (typeof window !== 'undefined') {
                window.dispatchEvent(new CustomEvent('performance_warning', {
                    detail: warningData
                }));
            }
    ...
    getAggregateMetrics() {
        const metrics = {
            ...this.aggregateMetrics,
            timestamp: new Date().toISOString(),
            errorRate: this.aggregateMetrics.totalRequests > 0 
                ? (this.aggregateMetrics.totalErrors / this.aggregateMetrics.totalRequests * 100).toFixed(2) 
                : 0,
    ...
    calculatePerformanceScore() {
        ...
        const errorPenalty = (this.aggregateMetrics.totalErrors / this.aggregateMetrics.totalRequests) * 30;
        const warningPenalty = (this.aggregateMetrics.warningCount / this.aggregateMetrics.totalRequests) * 15;
        const criticalPenalty = (this.aggregateMetrics.criticalCount / this.aggregateMetrics.totalRequests) * 25;

        const baseScore = 100;
        const finalScore = Math.max(0, Math.min(100, baseScore - errorPenalty - warningPenalty - criticalPenalty));
