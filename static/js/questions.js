    ...
    container.innerHTML = `
        <div class="question-card">
            <div class="question-header">
                ...
                <span class="difficulty-badge ${difficultyClass}">${question.difficulty.toUpperCase()}</span> <span class="score">Score: ${state.score}/${state.totalAnswered}</span>
            </div>
            ...
            <div class="question-actions">
                <button type="button" class="action-btn prev" onclick="previousQuestion()" ${state.currentQuestionIndex === 0 ? 'disabled' : ''}>← Previous</button>
                <button type="button" class="action-btn next" onclick="nextQuestion()" ${state.currentQuestionIndex >= state.questions.length - 1 ? 'disabled' : ''}>Next →</button>
            </div>
        </div>
    `;
