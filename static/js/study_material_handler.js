class StudyMaterialHandler {
    ...
    validateFile(event) {
        ...
        if (file.size > maxSize) {
            this.showError(`File size exceeds the maximum limit of 1MB (current size: ${(file.size/1024).toFixed(1)}KB)`);
            this.fileInput.value = '';
            return false;
        }
    ...
    showLoading() {
        if (this.uploadProgress) {
            ...
            this.uploadStatus.textContent = 'Uploading and processing study material... This may take a while for large files.';
        ...
        try {
            ...
            const analysisHtml = `
                <div class="analysis-results mt-3">
                    ...
                    ${this.renderAnalysisSection('NCLEX Categories', parsedAnalysis.nclex_categories)}
                    ${this.renderAnalysisSection('Learning Objectives', parsedAnalysis.learning_objectives)}
                    <div class="mt-2">
                ...
                ${questions.map((q, index) => `
                    <div class="question-card mt-2">
                        ...
                        <ul class="options-list">
                            ${q.options.map((opt, i) => `
                                <li class="${i === q.correct_answer ? 'correct-answer' : ''}">${opt}</li>
                            `).join('')}
                        </ul>
