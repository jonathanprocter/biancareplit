...
from models import db, Content, Review, StudyMaterial, StudentProgress, SubjectCategory, DifficultyLevel, ContentType # Added ContentType import
...
def get_analytics_dashboard():
    ...
    try:
        ...
        category_stats = db.session.query(
            ...
            func.sum(case((Review.is_correct == True, 1), else_=0)).label('correct_reviews')
        ).join(
            Review, Review.content_id == Content.id
        ).group_by(
            Content.category
        ).all()
        ...
        study_time = db.session.query(
            ...
            func.coalesce(func.sum(StudyMaterial.duration), 0).label('total_duration')
        ).group_by(
            StudyMaterial.category
        ).all()
        ...
        recent_performance = db.session.query(
            ...
            Review.created_at.cast(db.Date)
        ).order_by(
            Review.created_at.cast(db.Date).desc()
        ).limit(7).all()
...
def track_performance():
    ...
    try:
        ...
        required_fields = ['question_id', 'selected_answer', 'is_correct', 'category', 'difficulty']
    ...
    try:
        ...
        if progress:
            ...
            daily_stats = db.session.query(
                ...
                func.sum(case((Review.is_correct == True, 1), else_=0)).label('correct_answers'),
                func.sum(Review.study_duration).label('total_time')
            ).filter(
                Review.created_at.between(start_of_day, end_of_day)
            ).first()
        ...
        mastered_topics = db.session.query(
            Content.category,
            func.count().label('total'),
            func.sum(case((Review.is_correct == True, 1), else_=0)).label('correct')
        ).join(
            Review, Review.content_id == Content.id
        ).filter(
            Review.created_at.between(start_of_day, end_of_day)
            ...
            func.sum(case((Review.is_correct == True, 1), else_=0)) * 100.0 / func.count() >= 80
        ).all()
        
        mastered_topics_list = [topic[0].value for topic in mastered_topics] if mastered_topics else []
        
        # Format study time
        hours = int(study_time // 60)
        minutes = int(study_time % 60)
        time_spent = f"{hours}h {minutes}m" if hours > 0 else f"{minutes}m"
        formatted_duration = f"{hours} hours and {minutes} minutes" if hours > 0 else f"{minutes} minutes"
        avg_time = round((study_time * 60) / questions_attempted) if questions_attempted > 0 else 0

        # Generate recommendations
        recommendations = generate_recommendations(questions_attempted, accuracy_rate, study_time)
...
async def get_questions_by_category(category):
    ...
    try:
        ...
        logger.info(f"Request parameters - Category: {category}, Difficulty: {difficulty}, Force Generate: {force_generate}, Count: {count}")
        ...
        try:
            ...
            questions = [] if force_generate else Content.query.filter_by(
            ...
            if not questions or force_generate:
                logger.info(f"{'Forcing new question generation' if force_generate else 'No questions found'} for {category}")
                try:
                    ...
                    logger.info(f"Successfully generated {len(questions_data)} questions from AI")
                    ...
                    for q in questions_data:
                        try:
                            content = Content(
                                ...
                                keywords=q['keywords'],
                                clinical_scenario=q['question'] if 'patient' in q['question'].lower() else '',
                                nursing_interventions=[],
                                expected_outcomes=[]
                            )
                            new_questions.append(content)
                            logger.info(f"Created content object for question: {q['question'][:50]}...")
                        except Exception as e:
                            logger.error(f"Error creating content object: {str(e)}")
                            continue
                    
                    if not new_questions:
                        logger.error("Failed to create content objects from generated questions")
                        return jsonify({
                            'success': False,
                    ...
                    try:
                        ...
                        logger.info(f"Successfully saved {len(questions)} new questions to database")
                    except Exception as db_error:
                        logger.error(f"Database error while saving questions: {str(db_error)}")
                        db.session.rollback()
                        return jsonify({
                            'success': False,
        ...
        logger.info(f"Successfully loaded {len(questions_data)} questions for category: {category}")
        return jsonify({
            'success': True,
        ...
        logger.error(f"Error loading questions for category {category}: {str(e)}", exc_info=True)
        return jsonify({
            'success': False,
                ...
                with open(file_path, 'r') as file:
                    ...
                    for block in question_blocks:
                        ...
                        try:
                            ...
                            category_line = next((line for line in lines if 'NCLEX Category:' in line), '')
                            category_text = category_line.split('NCLEX Category:')[-1].strip() if category_line else ''
                            ...
                            for i, line in enumerate(lines):
                                if line.startswith(('A)', 'B)', 'C)', 'D)')):
                                    ...
                                    if not question_text and i > 0:
                                        # Skip header lines
                                        start_idx = next((j for j, l in enumerate(lines) if not l.startswith('Question')), 0)
                                        question_text = ' '.join(lines[start_idx:i]).strip()
                                    options.append(line[3:].strip())
                                elif line.startswith('Correct Answer:'):
                                    answer_letter = line.replace('Correct Answer:', '').strip()
                                    correct_answer = {'A': 0, 'B': 1, 'C': 2, 'D': 3}.get(answer_letter, 0)
                                elif line.startswith('Rationale:'):
                                    rationale = ' '.join(lines[i+1:]).strip()
                            
                            # Validate required fields
                            if question_text and len(options) == 4 and correct_answer is not None and rationale:
                                content = Content(
                                    type=ContentType.QUIZ,
                                    category=category_enum,
                                    ...
                                    rationale=rationale,
                                    keywords=[category_enum.value.lower(), difficulty_level.value.lower()],
                                    clinical_scenario=question_text if 'patient' in question_text.lower() else '',
                                    nursing_interventions=[],
                                    expected_outcomes=[]
                                )
                                ...
                                imported_count += 1
                                logger.info(f"Successfully parsed question {imported_count}")
                            else:
                                logger.warning("Skipped invalid question - missing required fields")
...
def generate_recommendations(questions_attempted, accuracy_rate, study_time):
    ...
    if questions_attempted < 10:
        recommendations.append("Try to attempt more practice questions to improve your understanding")
    
    if accuracy_rate < 70:
        recommendations.append("Review your incorrect answers and create flashcards for challenging topics")
    
    if study_time < 30:
        recommendations.append("Consider increasing your daily study time for better retention")
    
    if not recommendations:
        recommendations.append("Great work today! Consider trying some advanced topics to challenge yourself")
    ...
    try:
        ...
        if progress:
            summary_data.update({
                ...
                'accuracyRate': (progress.correct_answers / progress.questions_attempted * 100) if progress.questions_attempted > 0 else 0,
        ...
        try:
            ...
            summary_data['topicsMastered'] = [topic[0].value for topic in mastered_topics] if mastered_topics else []
            logger.info(f"Retrieved mastered topics: {summary_data['topicsMastered']}")
            
        except Exception as db_error:
            logger.error(f"Database error while fetching mastered topics: {str(db_error)}")
...
async def generate_questions():
    ...
    try:
        ...
        # Generate questions
        try:
            logger.info(f"Attempting to generate {count} {difficulty} questions for {category}")
            ...
            logger.error(f"Unexpected error during question generation: {str(e)}", exc_info=True)
            return jsonify({
                'success': False,
        ...
        try:
            ...
            logger.info(f"Successfully generated and saved {len(content_objects)} questions")
