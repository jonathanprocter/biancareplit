...
import { Trophy, Star, Award, Shield, Brain, Book, Target, Zap } from 'lucide-react';
              ...
              <div className="mt-2 space-y-2">
                ...
                <div className="mt-1">
                  <div className="flex justify-between text-xs mb-1">
                    ...
                    <span className="font-medium">{badge.earnedAt ? '100%' : Math.min(100, Math.round((progress?.totalPoints || 0) / badge.requiredPoints * 100))}%</span>
                  </div>
                  <div className="h-1.5 bg-primary/20 rounded-full overflow-hidden">
                    <motion.div 
                      className="h-full bg-primary rounded-full"
                      initial={{ width: 0 }}
                      animate={{ width: badge.earnedAt ? '100%' : `${Math.min(100, Math.round((progress?.totalPoints || 0) / badge.requiredPoints * 100))}%` }}
              ...
              return (
                <Card key={category} className="p-4">
                  <div className="text-center">
                    ...
                    <p className="text-sm text-muted-foreground">{earnedCount}/{categoryBadges.length}</p>
                  </div>
                </Card>
              );
            })}
