  ...
  initializationPromise = (async () => {
    ...
    for (let attempt = 1; attempt <= retries; attempt++) {
      try {
        ...
        if (attempt === retries) {
          throw new OpenAIServiceError(`Failed to initialize after ${retries} attempts: ${error instanceof Error ? error.message : String(error)}`);
        }
...
const getOpenAI = async (): Promise<OpenAI> => {
  try {
    ...
    throw error instanceof OpenAIServiceError ? error : new OpenAIServiceError('Failed to get OpenAI instance');
  }
};
...
export async function getMentorResponse(
  ...
  try {
    ...
    try {
      ...
      return {
        content: parsedContent.content || content || 'I apologize, but I couldn\'t generate a response. Please try again.',
      };
    } catch {
      return {
...
export async function generateLearningPath(
  ...
  try {
    ...
    try {
      ...
      throw new OpenAIServiceError('Failed to parse OpenAI response: ' + (parseError instanceof Error ? parseError.message : String(parseError)));
    }
  } catch (error) {
    console.error('Error generating learning path:', error);
    throw new OpenAIServiceError(error instanceof Error ? error.message : 'Failed to generate learning path');
  }
}