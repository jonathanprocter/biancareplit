...
const initializeOpenAI = async (retries = 3): Promise<OpenAI> => {
  ...
  initializationPromise = (async () => {
    ...
    for (let attempt = 1; attempt <= retries; attempt++) {
      try {
        console.log(
          `Initializing OpenAI client (attempt ${attempt}/${retries})...`,
        );
        ...
        if (!response?.choices?.[0]?.message?.content) {
          throw new OpenAIServiceError(
            'Invalid response during API key verification',
          );
        }
        ...
        console.error(
          `Failed to initialize OpenAI client (attempt ${attempt}/${retries}):`,
          error,
        );
    ...
    const systemMessage = {
      ...
      content: `You are an expert NCLEX tutor and mentor. ${
        context?.currentTopic
          ? `The student is currently studying ${context.currentTopic}.`
          : ''
      } ${
