...
from models import Content, db, ContentType, SubjectCategory, DifficultyLevel
import re
import logging
...
def parse_question_block(text):
    ...
    for line in lines:
        ...
        if line.startswith('Question'):
            ...
            match = re.search(r'Question\s+\d+\s*\((\w+)[^)]*;\s*NCLEX\s+Category:\s*([\w\s-]+)', line, re.IGNORECASE)
            if match:
                ...
                for block in question_blocks:
                    ...
                    try:
                        ...
                        if not question_data['question'] or not question_data['options']:
                            logger.warning("Skipping question with missing required fields")
                            continue
                        
                        logger.info(f"Importing question: Category={question_data['category']}, Difficulty={question_data['difficulty']}")
                        
                        # Create content entry
                        content = Content(
                            type=ContentType.QUIZ,
                            category=SubjectCategory[question_data['category']],
                            difficulty=DifficultyLevel[question_data['difficulty'].upper()],
