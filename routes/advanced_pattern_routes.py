...
from models.advanced_pattern_recognition import AdvancedPatternRecognition, AdvancedPatternModel
...
@advanced_pattern_routes.route('/api/advanced-patterns/user/<int:user_id>', methods=['GET'])
def get_user_patterns(user_id):
    """Get stored pattern analysis for a user"""
    try:
        patterns = AdvancedPatternModel.query.filter_by(user_id=user_id)\
            .order_by(AdvancedPatternModel.created_at.desc())\
            .limit(10)\
            .all()
        
        return jsonify({
            'patterns': [pattern.pattern_data for pattern in patterns]
        })
    except Exception as e:
        return jsonify({'error': str(e)}), 500

@advanced_pattern_routes.route('/api/advanced-patterns/recommendations/<int:user_id>', methods=['GET'])
def get_recommendations(user_id):
    """Get personalized learning recommendations based on pattern analysis"""
    try:
        # Get latest patterns
        latest_pattern = AdvancedPatternModel.query\
        ...
        return jsonify({
            'recommendations': latest_pattern.pattern_data.get('deep_patterns', {}).get('recommended_adjustments', {})
        })
    except Exception as e:
        return jsonify({'error': str(e)}), 500
