...
def get_performance_data():
    ...
    try:
        ...
        mastered_topics = sum(1 for rate in patterns['topic_mastery'].values() if rate >= 80)
        ...
        category_performance = db.session.query(
            ...
            func.sum(case([(Review.is_correct, 1)], else_=0)).label('correct_answers')
...
def generate_adaptive_content():
    ...
    try:
        ...
        recent_performance = Review.query.filter_by(user_id=user_id)\
            .order_by(Review.created_at.desc())\
            .limit(10)\
            .all()
        ...
        content = adaptive_system.generate_adaptive_content(
            ...
            preferred_categories=weak_areas
        )
        ...
        avg_question_time = db.session.query(func.avg(Review.time_taken))\
            .filter_by(user_id=user_id)\
            .scalar() or 60  # Default to 60 seconds if no data
        ...
        return jsonify({
            ...
            'adaptiveHints': [
                hint for hint in content.hints
...
def submit_answer():
    ...
    try:
        ...
        adaptive_system = AdaptiveLearningSystem(db_connection=db)
        adaptive_system.track_response_patterns(user_id, {
            'is_correct': is_correct,
            'time_taken': time_spent,
            'content_id': content_id
        })
...
def review_flashcard():
    ...
    if pattern:
        pattern.accuracy_rate = (pattern.accuracy_rate * pattern.pattern_data.get('total_reviews', 0) + (quality >= 3)) / (pattern.pattern_data.get('total_reviews', 0) + 1)
        pattern.pattern_data['total_reviews'] = pattern.pattern_data.get('total_reviews', 0) + 1
